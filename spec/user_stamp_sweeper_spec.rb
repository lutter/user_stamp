...
class FauxApplicationController; end;
...
describe UserStampSweeper, "#before_validation" do
  ...
  describe "(with new record)" do
    ...
    it "should NOT set creator_id if attribute does not exist" do
      record = mock('Record', :new_record? => true, :updater_id= => nil, :respond_to? => false, :changed? => true)
  ...
  describe "(with non new record)" do
    ...
    describe "(when the record has not changed)" do
      it "should not set updater_id if attribute exists" do
        record = mock('Record', :creator_id= => nil, :updater_id= => nil, :new_record? => :false, :changed? => false)
        record.should_not_receive(:updater_id=)
        @sweeper.before_save(record)
      end
    end

    describe "(when the record has changed)" do
      it "should set updater_id if attribute exists" do
        record = mock('Record', :creator_id= => nil, :updater_id= => nil, :new_record? => :false, :changed? => true)
        record.should_receive(:updater_id=)
        @sweeper.before_save(record)
  ...
  it "should NOT set updater_id if attribute does not exist" do
    record = mock('Record', :creator_id= => nil, :updater_id= => nil, :new_record? => :false, :respond_to? => false, :changed? => true)
...
describe UserStampSweeper, "#before_validation (with custom attribute names)" do
  ...
  describe "(with new record)" do
    ...
    it "should NOT set created_by if attribute does not exist" do
      record = mock('Record', :new_record? => true, :updated_by= => nil, :respond_to? => false, :changed? => true)
...
<<<<<<< HEAD:spec/user_stamp_sweeper_spec.rb
  it "should set updated_by if attribute exists" do
...
>>>>>>> don't stamp an existing record if it's not changing:spec/user_stamp_sweeper_spec.rb
  end
